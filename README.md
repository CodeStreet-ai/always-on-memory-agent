...
Each agent has its own tools for reading/writing the memory store. The orchestrator routes incoming requests to the right specialist via handoffs.
...
```
always-on-memory-agent/
├── agent.py            # Always-on agent (the real thing)
