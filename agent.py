"""
Agent Memory Layer — Always-On Agent

A lightweight, cost-effective background agent that continuously processes, consolidates, and serves memory. Runs 24/7 on a fast, cheap model.
...
# The deployment name on your Azure OpenAI resource (from LLM_MODEL=azure_openai:gpt-4.1)
MODEL = os.getenv("MODEL", "gpt-4.1")
DB_PATH = os.getenv("MEMORY_DB", "memory.db")

# Azure OpenAI configuration. Load these from a git-ignored .env file.
...
def configure_openai_provider() -> None:
    """Point the Agents SDK at Azure OpenAI when configured.

    Azure OpenAI speaks the OpenAI Responses API; we reach it by registering a
    default OpenAI client whose base_url is the resource endpoint and whose
    api-version is sent as a query parameter. When AZURE_OPENAI_ENDPOINT is not
    set the SDK falls back to a standard OpenAI client.
    """
    ...
    # Azure OpenAI expects the "/openai" suffix on the resource URL.
    ...
    # Azure OpenAI has no OpenAI tracing endpoint; disable tracing to avoid a 401 on api.openai.com
...
def build_agents():
    ingest_agent = Agent(
        ...
        tools=[function_tool(store_memory)],
    )
    ...
    consolidate_agent = Agent(
        ...
        tools=[function_tool(read_unconsolidated_memories), function_tool(store_consolidation)],
    )
    ...
    query_agent = Agent(
        ...
        tools=[function_tool(read_all_memories), function_tool(read_consolidation_history)],
    )
    ...
    orchestrator = Agent(
        ...
        tools=[function_tool(get_memory_stats)],
     )
...
class MemoryAgent:
    ...
    def _media_input_item(mime_type: str, file_bytes: bytes, filename: str = "") -> dict:
        """Build a multimodal input item for the OpenAI Responses API."""
        b64 = base64.b64encode(file_bytes).decode("utf-8")
        data_url = f"data:{mime_type};base64,{b64}"
        if mime_type.startswith("image/"):
            return {"type": "input_image", "image_url": data_url, "detail": "auto"}
        # `filename` is required alongside `file_data` for input_file items.
    ...
    async def ingest_file(self, file_path: Path) -> str:
        ...
        # Most models have a ~20MB inline limit; skip very large files
        if size_mb > 20:
        ...
        try:
            for f in sorted(folder.iterdir()):
                ...
                try:
                    ...
                    continue  # leave unmarked so it's retried next poll
...
def main():
    parser = argparse.ArgumentParser(description="Agent Memory Layer - Always-On Agent")
